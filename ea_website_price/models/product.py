...
class Product(models.Model):
    ...
    website_price = fields.Float(
        'Website price', compute='_website_price', digits=dp.get_precision('Product Price'))
    ...
    website_price_difference = fields.Boolean(
        'Website price difference', compute='_website_price')
    ...
    def _website_price(self):
        ...
        for p, p2 in pycompat.izip(self, self2):
            ...
            p.website_price = taxes.compute_all(
                p2.price, pricelist.currency_id, quantity=qty, product=p2, partner=partner)[ret]*1.21
            ...
            p.website_price_difference = True if float_compare(
                price_without_pricelist, p.website_price, precision_rounding=pricelist.currency_id.rounding) > 0 else False
            p.website_public_price = taxes.compute_all(
                p2.lst_price, quantity=qty, product=p2, partner=partner)[ret]*1.21
            print('p', p)
            print('p.website_price', p.website_price)
            _logger.info('p.website_price %s' % (p.website_price))
            # 1/0
